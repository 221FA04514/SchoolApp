...
fixList.forEach(item => {
  ...
  Widget`;
    ...
    if (appbarRegex.test(code)) {
        console.log(`Re-fixing appbar in ${item.file}`);
        code = code.replace(appbarRegex, newAppBar);

        // Ensure only one Clipper at the end
        const clipperRegex = /\nclass _HeaderClipper[\s\S]*$/m;
        if (code.match(/class _HeaderClipper/g).length > 1) {
            code = code.replace(clipperRegex, '');
            code += `
...
`;
        ...
        console.log(`COULD NOT FIND APPBAR IN ${item.file}`);
    }
});
